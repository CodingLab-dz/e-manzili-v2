    ...
    return (
        <div className="mb-5">
            ...
            <div className="relative rounded-2xl">
                ...
                <select className="w-full bg-[#07111f] border border-[#1e2535] rounded-xl pl-10 pr-10 py-3 text-sm text-white outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/10 transition-all appearance-none cursor-pointer">
                    <option value="" disabled className="text-[#3d4a5c]">Sélectionnez une option</option>
                    {options.map((opt) => (
                        <option key={opt} value={opt} className="bg-[#07111f] text-white">{opt}</option>
                    ))}
                </select>
                {/* Custom chevron */}
...
export default function Formacadelear() {
