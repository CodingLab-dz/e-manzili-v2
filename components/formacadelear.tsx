    ...
    return (
        <div className="mb-5">
            ...
            <div className="relative">
                <i
                    className={`ti ${icon} absolute left-3.5 ${textarea
                        ? "top-3.5"
                        : "top-1/2 -translate-y-1/2"
                        } text-[#3d4a5c] text-base pointer-events-none`}
                />
    ...
    return (
        <div className="mb-5">
            ...
            <div className="relative rounded-2xl">
                <i className={`ti ${icon} absolute left-3.5 top-1/2 -translate-y-1/2 text-[#3d4a5c] text-base pointer-events-none z-10`} />

                <select
                    value={value}
                    onChange={onChange}
                    className="w-full bg-[#07111f] border border-[#1e2535] rounded-xl pl-10 pr-10 py-3 text-sm text-white outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/10 transition-all appearance-none cursor-pointer"
                >
                    <option value="">
                ...
                <i className="ti ti-chevron-down absolute right-3.5 top-1/2 -translate-y-1/2 text-[#3d4a5c] text-base pointer-events-none" />
            </div>
        </div>
    );
}

const FormCom = ({ service }: { service: string }) => {
        ...
        <form onSubmit={handleSubmit}>
            <div className="bg-[#0d1520] border border-[#1e2535] rounded-3xl p-10 max-w-xl mx-auto font-inter" data-aos="zoom-in" data-aos-duration="1500">
                ...
                <SelectField
                    ...
                    onChange={(e) =>
                        setForm({ ...form, formation: e.target.value })
                    }
                />
                <Field
                    label="Message"
                ...
                <button type="submit" className="w-full bg-cyan-400 text-[#07111f] rounded-xl py-3.5 font-syne font-bold text-base tracking-wide hover:opacity-90 active:scale-[0.98] transition-all mt-2">
                    Envoyer la demande
                </button>
...
export default function Formacadelear({ service }: { service: string }) {

    return <Suspense>
        <FormCom service={service} />
    </Suspense>
}